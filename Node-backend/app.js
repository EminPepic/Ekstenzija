...
function deepClone(value) { return value === null || value === undefined ? value : JSON.parse(JSON.stringify(value)); }
...
function evaluate(value, response, responseText) {
  ...
  if ([413, 414, 415].includes(response.status)) return { status: "Passed", note: `Server je odbio payload (${response.status}).`, message: "Payload blokiran, zaštita radi." };
...
function runAutocannon(options) {
  return new Promise((resolve, reject) => {
    // Ne pozivamo autocannon.track – neće ništa ispisivati
    ...
    // autocannon.track(instance, { renderProgress: false }); // UKLONJENO
  });
}
